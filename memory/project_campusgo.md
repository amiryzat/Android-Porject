...
**Edge-to-edge / insets**: targetSdk 36 enables edge-to-edge automatically on Android 15+. All screens use `InsetsHelper.applyTopInsetPadding()` (see `InsetsHelper.kt`) on their top header/toolbar container. FeedFragment uses `android:fitsSystemWindows="true"` on CoordinatorLayout + AppBarLayout instead. Login/Register apply insets to the root ScrollView (hardcoded `layout_marginTop` replaced with 16dp).

**Chat creation rule**: Only the runner creates the `/chats/{chatId}` document. The runner-side `ChatActivity.ensureChatExists()` is called immediately after `loadTask()` resolves. If the poster opens a chat, the doc already exists (they only reach ChatActivity via ChatListFragment or TaskTrackingActivity). `TaskDetailActivity.ensureChatExists()` also creates the doc when the runner clicks Negotiate — both paths are covered.
